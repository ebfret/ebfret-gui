function [method, thresholds] = remove_bleaching_dlg()
    % global status;
    status = 0;
    ...
    if status 
        ...
        for v = 1:length(vars)
            ...
            if get(self.(checkfld), 'value')
                thresholds.(vars{v}) = str2num(get(self.(editfld), 'string'));
            else
                thresholds.(vars{v}) = NaN;
            end
        end
    else
        method = -1;
        thresholds.fret = NaN;
        thresholds.donor = NaN;
        thresholds.acceptor = NaN;
    end
    delete(self.dialog);
end